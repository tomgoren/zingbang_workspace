...
### Claude Code / AI Agent Conventions

- Use `git -C /absolute/path/to/repo` instead of `cd /path && git ...` in Bash tool calls and agent prompts. The `cd && git` compound form triggers a hardcoded bare-repository attack warning that cannot be suppressed via permission rules, even with `Bash(cd:*)` in the allow list.
